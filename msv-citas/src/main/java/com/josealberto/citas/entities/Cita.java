package com.josealberto.citas.entities;
...
public class Cita {
	...
	public void actualizarEstadoCita(EstadoCita nuevoEstadoCita) {
		...
		case PENDIENTE -> {
				if( !(nuevoEstadoCita.equals(EstadoCita.CONFIRMADA) ||
						nuevoEstadoCita.equals(EstadoCita.CANCELADA) ||
						nuevoEstadoCita.equals(EstadoCita.PENDIENTE)) )
		...
		case CONFIRMADA -> {
			if( !(nuevoEstadoCita.equals(EstadoCita.EN_CURSO) ||
					nuevoEstadoCita.equals(EstadoCita.CANCELADA) ||
					nuevoEstadoCita.equals(EstadoCita.CONFIRMADA)) )
				throw new IllegalStateException("La cita con estado " + this.estadoCita +
						" solo puede cambiar a " + EstadoCita.EN_CURSO + " o " + EstadoCita.CANCELADA);
			}
		case EN_CURSO -> {
				if(!nuevoEstadoCita.equals(EstadoCita.FINALIZADA))
					throw new IllegalStateException("La cita con estado " + this.estadoCita + 
							"solo puede cambiar a " + EstadoCita.FINALIZADA);
			}
		case FINALIZADA, CANCELADA -> {
			if(!nuevoEstadoCita.equals(this.estadoCita))
				throw new IllegalStateException("La cita con estado " + this.estadoCita + 
	...
	private void puedeEliminar() {
		if(this.estadoCita.equals(EstadoCita.CONFIRMADA) ||
		   this.estadoCita.equals(EstadoCita.EN_CURSO)) {
	...
	private void puedeActualizar() {
		if(!this.estadoCita.equals(EstadoCita.PENDIENTE)) {
			throw new IllegalStateException("La cita solo puede actualizarse si esta en estado de: " + 
